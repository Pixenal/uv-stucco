...
static
Result mapToFaces(MappingJobVars *pVars, FaceCellsTable *pFaceCellsTable) {
	Result err = STUC_SUCCESS;
	for (I32 i = pVars->inFaceRange.start; i < pVars->inFaceRange.end; ++i) {
		...
		if (inFace.size <= 4) {
			...
			err = mapPerTile(pVars, &inFace, pFaceCellsTable, i);
		}
		else {
			skipped = true;
